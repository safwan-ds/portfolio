...
export default function SectionWrapper({
  ...
  return (
    <section id={id} className={`relative py-24 md:py-32 px-4 sm:px-6 ${className}`}>
      <div className={`mx-auto max-w-${maxWidth} rounded-3xl bg-void/70 backdrop-blur-md border border-slate/10 p-8 md:p-12 pointer-events-auto`}>
        {label && title && (
          <SectionReveal delay={headerDelay}>
            <p className="font-mono text-xs tracking-[0.2em] text-neon-blue uppercase mb-3">{label}</p>
            <h2 className="font-display text-3xl md:text-4xl font-bold text-text-primary mb-8">{title}</h2>
          </SectionReveal>
        )}
        {children}
      </div>
