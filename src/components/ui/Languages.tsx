            ...
            return (
              <GlassCard key={lang.key}>
                <div className="flex items-center gap-4">
                  {/* Language name */}
                  <p className="font-display text-base font-semibold text-text-primary w-36 shrink-0 flex items-center gap-2">
                    <FlagIcon lang={lang.key as 'arabic' | 'english' | 'turkish'} className="w-6 h-4 rounded-sm" />
                    <span>{t(`languages.items.${lang.key}`)}</span>
                  </p>
                  {/* Bar track */}
                  <div className="flex-1 h-3 rounded-full bg-slate/30 overflow-hidden">
                    <div
                  ...
                  {/* Level label */}
                  <p className="font-mono text-xs text-text-secondary w-28 text-right shrink-0">
